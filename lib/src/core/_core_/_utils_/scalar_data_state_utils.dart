...
class ScalarDataStateUtils {
  ...
  ///    - If this scalar depends on a parent scalar whose value is absent, transitions to `None`.
  ///    - If the parent value shifted identity, cached scalar value is evicted to `Pending.initial`.
  /// 2. **Filter Criteria Shift (`filterCriteriaChanged`)**:
  ///    - Transitions loaded scalar metrics to `LoadedStale(filterChanged)` preserving historical failures.
  ///    - Invalidate pending states to `Pending.filterChanged`.
  /// 3. **Incoming Domain Events (`hasIncomingEvent`)**:
  ///    - Marks active in-memory values as `LoadedStale(event)` without discarding previous error payloads.
